...
| CTL-06 | Context quarantine of untrusted customer text | `src/context/quarantine.py` | `docs/architecture.md` §3 (quarantine strategy description); `tests/` context tests |
...
| CTL-12 | Resilient tool wrapper (timeout/retry, graceful degradation) | `src/tools/resilience.py` (`resilient_ainvoke`, `tool_failure`) | `notes/failures.local.md` (documented real timeout/retry incidents) |
...
| CTL-15 | Hard recursion-limit guard + graceful degrade | `src/config.py` (`recursion_limit`); `src/cli.py` (`_invoke_turn`, catches `GraphRecursionError`) | `notes/failures.local.md` (real recursion-limit incident, Phase 3) |
